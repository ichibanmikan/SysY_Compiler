...
void basic_blocks_gen(Function* func, syntax_tree_node* node);
//基本块生成。他用以生成if、while、函数体中的内容
//它读入的是stmts节点，但是他本身不对stmts生成一条一条的指令，而是交由basic_cmds_gen进行处理

void basic_cmds_gen(Function* func, BasicBlock* bb, syntax_tree_node* node);
...
void const_declartion_assignment_gen(Function* func, BasicBlock* bb, syntax_tree_node* node);
//常量生成语句
//和变量声明语句同样的注意
